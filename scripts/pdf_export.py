...
def build_pdf(res: dict, include_photo: bool = True,
    ...
    pdf.f(7.8, color=(12, 74, 110))
    pdf.multi_cell(CONTENT_W - 9, 3.8, note)
    y += nh + 7
    ...
    pdf.rect(MARGIN, y, CONTENT_W, 13, "F")
    pdf.set_xy(MARGIN + 5, y + 1.8)
    pdf.f(8.6, True, rec_fg)
    pdf.multi_cell(CONTENT_W - 10, 4.2, rec)
    y += 17

    # ══════ ภาคผนวก: ค่าเชิงเทคนิค ══════
    pdf.set_xy(MARGIN, y)
    pdf.f(8, True, SLATE)
    pdf.cell(0, 5, T("ภาคผนวก — ค่าทางเทคนิค", "Appendix - Technical values"), ln=1)
    tech = [
    ...
    for i, (k, v) in enumerate(tech):
        col = i % 2
        pdf.set_xy(cx + col * (CONTENT_W / 2), cy + (i // 2) * 4.6)
        pdf.f(7.2, color=SLATE)
        pdf.cell(34, 4.4, k)
        pdf.f(7.2, True, NAVY)
        pdf.cell(30, 4.4, v)
    y = cy + ((len(tech) + 1) // 2) * 4.6 + 4

    # ══════ Disclaimer ══════
    pdf.set_fill_color(255, 251, 235)
    pdf.rect(MARGIN, y, CONTENT_W, 16, "F")
    pdf.set_xy(MARGIN + 4, y + 2)
    pdf.f(7, color=(146, 64, 14))
    pdf.multi_cell(CONTENT_W - 8, 3.6, T(
        ...
        "a licensed physician or laboratory testing."))
