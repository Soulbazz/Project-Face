...
def build_model():
    base_model = models.vit_h_14(weights=None, image_size=518)
...
# 3. ครอบโค้ดหลักด้วย if __name__ == '__main__': เพื่อป้องกัน Multiprocessing Error บน Windows
if __name__ == '__main__':
    ...
    DATASET_CSV = "split_fixed_v2.csv"
    if not os.path.exists(DATASET_CSV):
        alt_paths = ["../split_fixed_v2.csv", "data/split_fixed_v2.csv", "../data/split_fixed_v2.csv"]
        for p in alt_paths:
            if os.path.exists(p):
                DATASET_CSV = p
                break
    ...
    print(f"[*] Loaded Test Set from: {DATASET_CSV} (Total samples: {len(test_df)})")

    test_dataset = FaceDataset(test_df, transform=transform)
    test_loader = DataLoader(test_dataset, batch_size=24, shuffle=False, num_workers=4, pin_memory=True)

    models_to_eval = [
        {"name": "Backup Epoch 7 (Run 1)", "path": "../weights/aug_epoch_7_backup.pt"},
        {"name": "Backup Epoch 7 (Run 2)", "path": "../weights/aug_epoch_7_backup2.pt"},
        {"name": "Audited Split V2 (Final)", "path": "../weights/vit_head_split_v2.pt"}
    ]

    results = []

    print("\n" + "="*65)
    print("              STARTING MODEL BENCHMARK EVALUATION")
    print("="*65)
    ...
    for m_info in models_to_eval:
        ...
        if not os.path.exists(m_path):
            print(f"[!] Warning: File {m_path} not found. Skipping...")
            continue
        ...
        elif "0.weight" in state_dict or "0.bias" in state_dict:
            model.heads.load_state_dict(state_dict, strict=False)
        else:
            model.load_state_dict(state_dict, strict=False)
    ...
    print("[*] Successfully saved results to: benchmark_results.csv\n")