...
4. (ทางเลือกเสริม) เปิดใส่ค่ารอบเอว (50–160 ซม.) เพื่อเพิ่มความแม่นยำในการคัดกรองระดับคลินิก
...
> ⚠️ **หมายเหตุสำคัญ:** `train_vit_bmi.py` เซฟไฟล์โมเดลที่เทรนเสร็จเป็นชื่อ `../weights/aug_epoch_7.pt` เสมอ (ไม่ว่าจะใส่ `--augmented` หรือไม่) แต่ `predict_pipeline.py` และแอปหลักโหลดโมเดลจากชื่อ `weights/vit_head_split_v2.pt` — หลังเทรนเสร็จต้อง **เปลี่ยนชื่อไฟล์เอง** ให้ตรงกัน มิฉะนั้นแอปจะยังใช้โมเดลตัวเก่าอยู่โดยไม่รู้ตัว
